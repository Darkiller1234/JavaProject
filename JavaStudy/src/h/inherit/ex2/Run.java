...
public class Run {
	 ...
	 * (실제로 상속된 클래스를 통해서 객체생성시 부모클래스의 객체도 생성이 돤다.
	 * 
	 * 상속의 장점
	 ...
	 * - 부모클래스에 정의되어있는 protected필드는 자식클래스에서 직접 접근이 가능하다(private 불가)
	 * - 자식객체는 부모클래스에 있는 메소드를 마치 내것처럼 호출할 수 있다.
	 * + 부모클래스에 정의된 기능이 마음에 들지 않는다면 내 마음대로 수정할 수 있다.(오버라이딩)
	...
	public static void main(String[] args) {
		...
		// 필드에는 int inch정보를 가지고 있고 informa--을 오버라이딩하여 기본 prod+inch정보 출력
		
		TV tv = new TV("삼성", "s-109", "올레드티비", 1000000, 50);
