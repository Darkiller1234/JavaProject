...
public class Run {
	 ...
	 * 
	 * ex) 학생 관리 프로그램
	 * 1) 학생관련 객체(~~~훈련생, 윤대한훈련생, ~~~훈련생, ...)
	 * 2) 공통적인 속성및 기능(이름, 나이, 주소, 전화번호, 키, 몸무게, 달리기속도, 먹는다, 잔다, 화장실간다, ...)
	 * 3) 학생 "인적사항" 관리하는 프로그램 : 이름, 나이, 주소, 전화번호, 반, ...
	 *    학생 "성적"을 관리하는 프로그램 : 이름, 반, 과목별 점수, ...
	 *    학생 "건강"을 관리하는 프로그램 : 이름, 반, 키, 몸무게, 달리기속도, 먹는다, 휴식한다, ...
	 * 4) 이름 : String name;
	 *    나이 : int age;
	 *    키  : double height;
	 * 
	 ...
	 *   	윤대한 이라는 훈련생 객체 하나 만들기 -> String name1 = "윤대한"; int age = 22; double height = 177.0;
	 *   	~~~  이라는 훈련생 객체 하나 만들기 -> String name1 = "윤대한"; int age = 19; double height = 180.0;
	 *   	
	 *   2. 배열로 프로그래밍 한다면
	 *   	배열 : 하나의 자료형으로 여러개의 값들을 보관할 수 있음
	 *   	이름을 보관하는 배열 String[] name = ("윤대한", "~~~", ...)
	 *   	나이를 보관하는 배열 int[] age = {22, 19, ...}
	 *   
	 *   	문제 -> 중간이 비어버릴 때마다 새로운 배열 생성 필요
	 *   
	 *   3. 그래서 나온 개념 -> 구조체
	 *   구조체 : 여러개의 자료형의 여러개의 값들을 보관 가능
	 *   String 값도 보관하고 int 값도 보관하고 double 값도 함께 보관하는 자료형을 직접 만든다고 생각 -> 나만의 자료형
	 *   String human{
	 *   	String name;
	 *   	int age;
	 *   	double height;
	 ...
	 * 		  외부로부터 "데이터접근을 막고"
	 * 		  대신에 "데이터를 간접적으로 처리(값을 변경하거나, 가져오는 것 등)"할 메소드들을
	 * 		  클래스 내부에 작성하여 관리하는 것
