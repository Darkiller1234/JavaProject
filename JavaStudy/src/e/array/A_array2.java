...
//		System.out.println(dArr.hashCode());
		...
		/*
		 * 실제 리터럴값을 곧바로 담을 수 ㅣ있는 변수를 일반변수라고 한다.
		 * 주소값을 담고 있는 변수는 참조 변수라고 표현(러페런스 변수
		 * 
		 * 기본자료형(원시타입)::boolean,int,char,byte,double등등으로 선언된 함수
		 * => 실제 리터럴값을 바로 담을 수 있다.
		 * 
		...
		int[] arr2 = {1,1,1,1,1};
		int[] arr3 = {1,1,1,1,1};
		System.out.println(arr2 == arr3); // false => 각 참조변수는 ==을 통해서 비교시 주소값을 비교함
