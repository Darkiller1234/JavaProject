...
public class TCPServer {
	...
	public static void main(String[] args) {
		...
		//1) 포트번화 지정(서버측에서 몇번 포트로 통로를 열것인가?)
		int port = 3000;
		
		try {
			//2) SeverSocket 객체생성시 포트와 결합(Bind)
			ServerSocket server = new ServerSocket(port);
			
			System.out.println("클라이언트 요청을 기다리고 있습니다.");
			
			//3) 요청을 기다렸다 요청이 오면 -> 요청 수락 후 클라이언트와 통신할 수 있는 서버쪽 소캣객체 생성
			...
			//PrintWriter : 데이터출력시 print(), println() 메소드를 가지고있는 보조 스트림
			while(true) {
