 ...
 *  -웹어서의 통신방식은 HTTP 프로토콜을 사용한다.
 *  
 *  *자바만가지고 서버와 클라이언트의 간단한 통신을 해보자
 *   이때 데이터를 입출력하고자 한다면 서버와 클라이언트간에 스트림(통로)이 필요함
 *   
 *   소캣 프로그래밍(TCP / UDP)
 *   - TCP방식 : 데이터의 전송속도가 느리다. 데이터가 정확하고 안정적으로 전달 가능(신뢰성이 요구되는 프로그램)
 *   - UDP방식 : 데이터의 전송속도가 빠르다. 신뢰성이 없는 데이터 전달에 용이(데이터를 빠른속도로 전송하고자 할때)
 *   
 */
...
public class Run {
	public static void main(String[] args) {
		try {
			...
			System.out.println("내 PC명 : " + localhost.getHostName());//호스트이름 반환
			System.out.println("내 IP주소 : " + localhost.getHostAddress());//호스트ip주소 반환
			...
			System.out.println("google 서버명 : " + googleHost.getHostName());//google이름 반환
			System.out.println("google IP주소 : " + googleHost.getHostAddress());//googleip주소 반환
			...
			for(InetAddress n : naverHost) {
				System.out.println("naver 서버명 : " + n.getHostName());//naver이름 반환
				System.out.println("naver IP주소 : " + n.getHostAddress());//naverip주소 반환
				System.out.println();
			}
