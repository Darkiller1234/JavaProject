...
public class Run {
	 ...
	 * 
	 * *JDBC용 객체 
	 * - Connection : DB의 연결정보를 담고있는 객체 - [Prepared]Statement : 연결된 DB에 sql문을 전달해서 실행하고, 결과를 받아내는 객체 
	 * - ResultSet : SELECT문 실행 후 조회된 결과물을 담는 객체
	 * 
	 * *JDBC 과정 
	 * 1) JDBC Driver 등록 : 해당 DMBS(오라클)가 제공하는 클래스 등록 
	 * 2) Connection 생성 : 연결하고자하는 db정보를 입력해서 해당 db와 연결할 수 있는 객체 
	 ...
	 * 
	 * 6_1) ResultSet객체에 담겨있는 데이터들을 하나하나 추출해서 옮겨담기[ArrayList에 담기] 6_2) 트랜잭션 처리(성공했으면
	 * commit, 실패했다면 rollback 실행)
	 * 
	...
	public static void main(String[] args) {
		...
		try {
			// 1) JDBC Driver 등록
			Class.forName("oracle.jdbc.driver.OracleDriver"); // ojdbc11.jar 파일을 추가하지 않거나 오타가 있으면 -> 에러 발생
			...
			// 4, 5)) sql문 전달하면서 실행 후 전달받기
			int result = stmt.executeUpdate(sql);
			//내가 실행할 sql문이 DML(INSERT, UPDATE, DELETE) -> stmt.executeUpdate(sql문) : int
			//내가 실행할 sql문이 select문 -> stmt.execute Query(select문) : ResultSet
			if(result > 0) {//요청성공
				conn.commit();
			}
			else {
